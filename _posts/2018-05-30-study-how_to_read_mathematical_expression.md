...
*  평균: $$ \mu = \frac{\sum_{i=1}^nx_i}{n} $$
*  분산 (편차 제곱의 평균): $$ \sigma^2 = \frac{\sum_{i=1}^n(x_i-\mu)^2}{n} $$
*  표준편차: $$ \sigma = \sqrt\frac{\sum_{i=1}^n(x_i-\mu)^2}{n} $$
...
*  계승: $$ n! = \Pi_{k=1}^nk=n\times(n-1)\times(n-2)\ \times...\times\ 3\times2\times1 $$
  *  그 수보다 작거나 같은 모든 양의 정수의 곱. 팩토리얼이라고 부름. !로 표시
*  $$ \Pi $$: 곱집합
...
* $$ \|a\| $$: 벡터의 norm, 벡터의 크기, 길이를 의미함
  * 예) 회귀모델의 coefficients(계수)가 w = (w_1, ..., w_p)라고 할 때, 편차의 제곱이 최소가 되는 w를 구하는 공식: $$ {min \atop w} {\|X w-y \|{_2}^2} $$
