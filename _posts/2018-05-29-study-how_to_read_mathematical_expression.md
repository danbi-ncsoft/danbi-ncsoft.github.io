...
* $ \|a\| $: 벡터의 norm, 벡터의 크기, 길이를 의미함
  * 예) 회귀모델의 coefficients(계수)가 w = (w_1, ..., w_p)라고 할 때, 편차의 제곱이 최소가 되는 w를 구하는 공식![coefficients_w](/assets/study/how_to_read_mathematical_expression\coefficients_w.png){: width="50" height="150"}
...
* $$ \int f(x)dx $$: 함수 f(x)의 적분
  ...
  * 예) 어떤 다변수 함수 f(x1,x2,...,xn)이 있을 때, f의 그레디언트(Gradient)는 아래처럼 표현
    ...
    ![gradient](/assets/study/how_to_read_mathematical_expression\gradient.png){: width="50" height="150"}
