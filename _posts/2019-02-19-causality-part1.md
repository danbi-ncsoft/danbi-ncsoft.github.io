...
이러한 문제를 "선택 편의(selection bias)"라고 부른다. 수식으로는 간단하지만 인과관계를 다루는 모든 문제의 근본 문제다 즉, 같은 개체에 대해서 두 개의 상황을 모두 겪게 한 차이와 원래 그렇게 선택한 집단만 모아서 평균을 낸 값은 일반적으로 같을 수 없다. 전자는 이른바 반사실(counterfactual)를 고려한, 즉 이상적인 실험 조건을 충족하는 상황이고 후자는 아니다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc1MzQzMTExOSwxODU0OTg2MDEyLC01Nj
UyMjY2NjAsMjM1NTQ4NDM0XX0=
-->