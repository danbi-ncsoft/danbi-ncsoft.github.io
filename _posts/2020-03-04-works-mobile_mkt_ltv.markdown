...
- 가정
  - $$Θ$$: 유저의 이탈 확률 , $$T=1$$ : 초기 시점
    - $$t$$ 시점에서 유저의 이탈 확률 $$\begin{aligned} P( T=t | Θ ) = Θ * (1-Θ)^{t-1} \end{aligned} $$
    - $$t$$ 시점에서 유저의 생존 확률: $$S( T=t \| Θ ) = (1-Θ)^t$$
    - 유저의 이탈 확률 $$Θ$$는 베타 분포를 따른다: $$f( Θ \| α , β ) = Θ^{α-1} * (1-Θ)^{β-1} / Β(α,β)$$
- 모델 설명
  ...
  - $$B(α , β) = Γ(α) * Γ(β) / Γ(α+β)$$ 와$$ Γ(α) = (α-1)!$$ 을 이용하면 유저의 $$t$$시점에서의 이탈확률은 다음과 같이 간단하게 나타낼 수 있음
    - $$P(T=1 \| α , β ) = α / (α+β) $$
